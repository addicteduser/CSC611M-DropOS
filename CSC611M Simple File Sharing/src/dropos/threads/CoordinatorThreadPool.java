 ...
 * One of the sixteen (16) instances of the {@link ToCoordinatorConnectionHandler} will then poll for the {@link Socket} and handle the connection.
 * @author Kevin
 *
 */
public class CoordinatorThreadPool {
	private final BlockingQueue<Socket> socketQueue;
	private ArrayList<ToCoordinatorConnectionHandler> threadList = new ArrayList<ToCoordinatorConnectionHandler>();
	...
	public CoordinatorThreadPool(int numThread){
		...
		for(int i = 0; i<numThread;i++){
			threadList.add(new ToCoordinatorConnectionHandler(socketQueue));
		}
